...
export const dashboardCategoriesAction = () => async (dispatch) => {
  try {
    dispatch({ type: "CATEGORY_LOADER" });
    const { data } = await api.get(`/public/categories`);
    dispatch({
...
export const addNewCategoryFromDashboard =
  (data, toast, reset, setLoader, setOpen) => async (dispatch) => {
    try {
      setLoader(true);
      ...
      toast.error(
        error?.response?.data?.description || "Failed to create category"
      );
    } finally {
      setLoader(false);
    }
...
export const deleteCategory =
  (toast, setLoader, categoryId, setOpenDeleteModel) =>
...
export const addToCart =
  ...
  (dispatch, getState) => {
    ...
    if (isQuantityExist) {
      ...
      toast.success(`${data?.productName} added to th cart!`);
...
export const updateOrderStatusFromDashboard =
  (orderId, orderStatus, toast, setLoader) => async (dispatch) => {
    try {
      setLoader(true);
...
export const addNewProductFromDashboard =
  (sendData, toast, reset, setLoader, setOpen) => async (dispatch) => {
    try {
      ...
      toast.error(
        error?.response?.data?.description || "Failed to create product"
      );
    } finally {
      setLoader(false);
    }
...
export const deleteProduct =
  (toast, setLoader, productId, setOpenDeleteModel) =>
  async (dispatch, getState) => {
    try {
      setLoader(true);
...
export const updateProductImageFromDashboard =
  (formData, toast, productId, setLoader, setOpen) => async (dispatch) => {
    try {
      setLoader(true);
      ...
      toast.error(
        error?.response?.data?.description || "Product Image upload fail"
      );
    }
  };
